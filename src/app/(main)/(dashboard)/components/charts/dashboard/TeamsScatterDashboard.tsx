...
function TeamsScatterDashboard({ teams, className }: Props) {
  const scatterData = teams.map((team) => ({
    ...
    logoUrl: getTeamLogoUrl(team.team, team.league),
  }));
  ...
  return (
    <Card className={cn("h-fit gap-2", className)}>
      ...
      <CardContent className="w-full h-[80vh] md:h-[700px] relative p-0">
        ...
        <div className="absolute top-2 left-2 font-bold text-muted-foreground z-10 text-xs">
          Weak & inconsistent
        </div>
        <div className="absolute bottom-2 right-2 font-bold text-muted-foreground z-10 text-xs">
          Strong & balanced
        </div>
        <div className="absolute bottom-2 left-2 font-bold text-muted-foreground z-10 text-xs">
          Weak but consistent
        </div>
...
/>
            ...
            <Scatter
              ...
              shape={(props: any) => {
                ...
                const radius = Math.max(z / 2, 16); // minimum size for logos
